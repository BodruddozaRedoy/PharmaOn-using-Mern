...
const SalesChannelChart = () => {
	...
	const SALES_CHANNEL_DATA = [
		...
		{ name: "Sellers", value: (users?.filter((user) => user.role === "seller" )).length },
		{ name: "Users", value: (users?.filter((user) => user.role === "user" )).length },
	];

	return (
		<motion.div
			className='mt-5 bg-white bg-opacity-50 backdrop-blur-md shadow-lg rounded-xl p-6 lg:col-span-2 border border-gray-700'
